...
class PlainTextKB:
    """
    Loads a data.txt-style KB.

    The supplied data.txt consists mostly of short natural-language facts and
    action/consequence sequences. This adapter:
      1. reads one non-empty line per statement;
      2. groups statements into episodes beginning with a non-robot state;
      3. stores every line;
      4. provides lexical retrieval for a query;
      5. extracts initial/final state pairs and intermediate robot actions.

    For a production system, replace lexical retrieval with a dense retriever
    (e.g. a Transformer embedding index) while keeping the same interface.
    """

    ACTION_RE = re.compile(r"^The robot\b", re.IGNORECASE)
    ...
    def _build_episodes(self):
        ...
        for line in self.lines:
            is_robot_action = bool(self.ACTION_RE.match(line))
            ...
            if is_robot_action:
                ...
                # A non-robot statement after actions is the resulting state.
                current.append(line)
    ...
    def retrieve(self, query: str, k: int = 5):
        """
        Simple lexical BM25-like retrieval without external dependencies.
        Returns [(score, episode), ...].
        """
...
class TextRecursiveBellmanPlanner:
    ...
    def _extract_answer(self, episode):
        """
        Convert a trajectory into an interpretable answer.

        Robot actions are returned in order.
        """
...
def text_planner_cli():
    ...
    if answer is None:
        print(
            "No relevant trajectory was found."
        )
        return
